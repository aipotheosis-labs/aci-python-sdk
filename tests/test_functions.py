...
def test_get_function_definition_success(client: Aipolabs) -> None:
    ...
    route = respx.get(f"{BASE_URL}functions/{function_name}/definition").mock(
        return_value=httpx.Response(200, json=mock_response)
    )

    response = client.functions.get_definition(function_name)
...
def test_get_function_definition_unauthorized(client: Aipolabs) -> None:
    ...
    with pytest.raises(AuthenticationError) as exc_info:
        client.functions.get_definition(function_name)
...
@respx.mock
def test_execute_function_success(client: Aipolabs) -> None:
    function_name = "TEST_FUNCTION"
